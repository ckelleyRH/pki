...
public class ESTFrontend {
    ...
    private Response cacerts(Optional<String> label) {
        ...
        return Response.ok(Base64.encodeBase64(out.toByteArray())).build();
    }
    ...
    public Response simpleenroll(byte[] data) throws Throwable {
        // This ignores non-base64 bytes.  Decoding will not fail, but if the
        // request body is not valid base64, it definitely won't decode to a valid
        // CSR, so CSR parsing will fail and we will still return 400.
        //
        PKCS10 csr;
        try {
